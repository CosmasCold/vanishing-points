const MEMORY_KEY = "bunker-command-memory";
...
export function logPlayerCommand(cmd: string) {
...
export function getProceduralGhostLines(): string[] {
  ...
  return [
    ...
    `Your ${item} is humming. You should check it.`,
    `Last time you typed "${lastCmd}". You will type it again.`,
    `"${favCmd}" is your favorite word. It will not save you.`,
    `The dust reads ${dust}%. It was ${Math.max(0, parseInt(dust) - 5)}% when you arrived.`,
    `You keep checking your dust. It will not save you.`,
    ...
    `Your favorite command is "${favCmd}". Predictable.`,
    `The static between "${lastCmd}" and "${mem.commands[1] || "nothing"}" is where I live.`,
    `You have been here ${hours} hours and still trust the terminal.`,
