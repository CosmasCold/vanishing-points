...
export const useInvestigationStore = create<InvestigationStore>((set) => ({
  ...
  updateEvidenceStatus: (invId, evId, status) => set((state) => ({
    evidence: {
      ...state.evidence,
      [invId]: state.evidence[invId]?.map((e) => 
  ...
  addTimelineEvent: (id, event) => set((state) => ({
    timelines: {
      ...state.timelines,
      [id]: [...(state.timelines[id] || []), event].sort((a, b) => 
  ...
  unlockMedia: (id, mediaId) => set((state) => ({
    unlockedMedia: {
      ...state.unlockedMedia,
