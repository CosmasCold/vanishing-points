...
export function registerNotebookCommands(registry: CommandRegistry) {
  registry.register({
    ...
    handler: async (args: string[]): Promise<CommandResult> => {
      ...
      // CASE 2: Append a new text segment (FIXED WRITE LOOP)
      if (writeIndex !== -1 || (args[0] === '--write' && args.length > 1)) {
        ...
        if (textContent) {
          ...
          if (isForbidden) {
            return {
              output: `OBSERVATION RECOGNIZED // CAUTION ADVISED\n------------------------------------------------\nObservation: "${textContent}"\n\nBUNKER_7: Entries contain variables locked under cognitive drift protocols. The keys are striking without ink. The carriage is writing to an empty field.`,
              type: 'warning',
            };
          }

          return {
            output: `OBSERVATION RECORDED: "${textContent}"\n------------------------------------------------\nObservation logged successfully in archival memory banks.`,
            type: 'success',
          };
        }
      }
      ...
      const formatted = [
        ...
        '================================================',
        ...notes.map((note, index) => `[Observation #${String(index + 1).padStart(2, '0')}]: ${note}`),
        '================================================',
